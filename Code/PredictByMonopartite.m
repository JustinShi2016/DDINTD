...
for k_fold = 1:nCV
    % split.
    Adj_temp = double(tenmat(Adj,1));
    CV_Temp= CV_Idx_row;    CV_Temp(k_fold) = [];
    ID_Trn_r_1 =  cell2mat(CV_Temp) ;
    N = size(ID_Trn_r_1,1);
    ID_Trn_r_2 = ones(N,1) * N + ID_Trn_r_1;
    ID_Trn_r = [ID_Trn_r_1;ID_Trn_r_2];

    clear CV_Temp;
    ID_Tst_r =  CV_Idx_row{k_fold} ;
    
        TrnLabelMat= Adj_temp(ID_Trn_r_1,ID_Trn_r); % may contain all-zero columns, which cause S4 prediction
        TstLabelMat = Adj_temp(ID_Tst_r,ID_Trn_r);
    ...
    switch upper(type)
		...
		case 'PGM' % KuiTao's implementation
		    Predicted_LabelMat =PredictS2_By_MatrixFactorization...
                (TrnLabelMat,FeatureMat(ID_Trn_r_1,:), TstLabelMat, FeatureMat(ID_Tst_r,:),  option_);
    end
    [AUC_(k_fold), AUPR_(k_fold)] = Measure_S2(Predicted_LabelMat,double(tenmat(TrnLabelMat,1)),double(tenmat(TstLabelMat,1)), true); 
...
TrueScore =  Scores( Label_mat(:)>0) ; % LOG:change it to adapt 1 binary-class
